-- CreateTable
CREATE TABLE "applicant_password_reset_tokens" (
    "id" TEXT NOT NULL,
    "token" TEXT NOT NULL,
    "email" TEXT NOT NULL,
    "expires_at" TIMESTAMP(3) NOT NULL,
    "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT "applicant_password_reset_tokens_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE UNIQUE INDEX "applicant_password_reset_tokens_token_key" ON "applicant_password_reset_tokens"("token");

-- CreateIndex
CREATE INDEX "applicant_password_reset_tokens_email_idx" ON "applicant_password_reset_tokens"("email");

-- CreateIndex
CREATE INDEX "applicant_password_reset_tokens_expires_at_idx" ON "applicant_password_reset_tokens"("expires_at");
